...
function mdDataGrid () {
  ...
  function MdDataGridController ($scope, $q, $timeout, $translate, $mdToast) {
    var vm = this
    ...
    function addFilter () {
      ...
      switch (newFilter.format) {
        case 'date':
          if (vm.dateFrom && vm.dateTo) {
            newFilter.label = $translate.instant(vm.options.layout.labels[vm.filterField]) +
                              '(' + vm.sortOrder + ')' + '[' + vm.dateFrom + ',' + vm.dateTo + ']'
            valid = true
          }
          break
        default:
      ...
      if (valid) {
        ...
        $mdToast.show(
          $mdToast.simple()
            .textContent($translate.instant('GRID.FILTER.INVALID'))
